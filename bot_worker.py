...
def bot_engine(state: dict, lock: threading.Lock, dur_sinyali: threading.Event):
    ...
    def _baglanti_kur():
        ...
        for attempt in range(5):
            try:
                # V42 Stage 1: Connection
                with lock:
                    log_ekle("🔄 Binance Futures'a bağlanılıyor...", state)
                ...
                for _mkt_attempt in range(3):
                    try:
                        ...
                        with lock:
                            log_ekle(f"✅ Piyasa verileri yüklendi ({len(exchange.markets)} market)", state)
                        break
                ...
                if _acquired:
                    try:
                        if not state.get("rest_connected_logged"):
                            log_ekle(f"🚀 Trading için hazır! (Futures REST API, defaultType: {getattr(cfg, 'FUTURES_TYPE', 'future')})", state)
