"""
Kripto Paper-Trading Bot — AI Dashboard
========================================
ccxt, pandas, streamlit ve AI karar motoru (ai_engine)
entegrasyonu ile sanal alım/satım simülasyonu.
"""
...
# ─────────────────────────────────────────────
st.set_page_config(
    page_title="PeroTrade AI — Akıllı Kripto Bot",
    page_icon="🧠",
    layout="wide",
...
st.markdown("""
<style>
.stApp { background: linear-gradient(135deg, #0b0c10 0%, #1f2833 100%); color: #c5c6c7; }
[data-testid="stMetric"] { background: rgba(31, 40, 51, 0.8); border: 1px solid #45a29e; border-radius: 12px; padding: 16px; box-shadow: 0 4px 15px rgba(0,0,0,0.4); }
...
def log_ekle(mesaj: str, state):
    zaman = datetime.now(timezone.utc).strftime("%H:%M:%S")
    state["ai_dusunce_gunlugu"].insert(0, f"[{zaman}] {mesaj}")
    # En fazla 50 log tut
        ...
        try:
            ...
            with lock:
                if state["pozisyon"] == "YOK":
                    log_ekle("🔍 Piyasa taranıyor... (Top volumenli coinler aranıyor)", state)
            
            # 1) Coin Seçimi
            if state["pozisyon"] == "YOK":
                # Boştayken hacimli coinleri tara
                top_coinler = ai_engine.top_coinleri_tara(exchange, limit=10)
                secilen_sembol = None
                en_iyi_skor = -999
                taranan_liste = []
                
                # Her coini analiz et 
                # (Rate limit'e takılmamak için 3 coini detaylı inceliyoruz)
                for coin in top_coinler[:3]:
                    df = ai_engine.mum_verisi_cek(exchange, coin, "1h", limit=preset["sma_uzun"]+5)
                    pazar = ai_engine.pazar_durumu_cikar(df, coin)
                    sma_sinyal = ai_engine.sinyal_uret(df, preset["sma_kisa"], preset["sma_uzun"])
                    skor = ai_engine.kompozit_skor_hesapla(pazar, sma_sinyal)
                    
                    taranan_liste.append({"Sembol": coin, "Fiyat": pazar["fiyat"], "Skor": round(skor,1), "Volatilite": round(pazar["volatilite"],2)})
            ...
            try:
                ...
                fiyat, degisim, hacim = ticker.get("last", secilen_pazar["fiyat"]), ticker.get("percentage", 0), ticker.get("quoteVolume", 0)
            except Exception:
                fiyat, degisim, hacim = secilen_pazar["fiyat"], 0, 0
            ...
            # 3) UI Güncelleme ve İşlem Yürütme
            with lock:
                state["fiyat"] = fiyat
                state["degisim_24s"] = degisim
                state["hacim_24s"] = hacim
                
                log_ekle(f"🧠 {secilen_sembol} Analizi → {karar_paketi['dusunce']}", state)
                ...
                if sinyal == "AL" and state["pozisyon"] == "YOK":
                    ...
                    log_ekle(f"💰 İŞLEM: {secilen_sembol} ALINDI. Fiyat: {fiyat:.2f}", state)
                    ...
                    log_ekle(f"💸 İŞLEM: {secilen_sembol} SATILDI. KZ: %{kz:+.2f}", state)
            ...
            # 4) Dinamik Bekleme Süresi
            bekleme_suresi = int(karar_paketi["aralik_sn"] * preset["aralik_carpan"])
            with lock:
                state["sonraki_analiz_sn"] = bekleme_suresi
                log_ekle(f"⏱️ Volatilite bazlı sonraki analiz aralığı: {bekleme_suresi} saniye.", state)
            ...
            with lock:
                log_ekle(f"❌ Hata oluştu: {str(e)}", state)
            time.sleep(10) # Hatada 10s bekle tekrar dene
...
def durdur():
    ...
    if "Hedefe" not in st.session_state.bot_durumu:
...
with st.sidebar:
    ...
    if ai_mod == "OpenAI LLM":
        ...
        st.caption("Key girilmezse veya kota biterse otomatik Mock AI'ye düşer.")
...
col_baslik.markdown("<h1 style='color: #66fcf1; font-weight: 800; margin-bottom: 0;'>🧠 PeroTrade AI</h1>", unsafe_allow_html=True)
col_durum.markdown(
    f"<div style='text-align:right; margin-top:20px;'>"
    f"<span class='status-badge {'status-running' if st.session_state.bot_calisiyor else 'status-stopped'}'>"
...
with col_sol:
    ...
    st.markdown("<div class='dashboard-header'><b>🔍 Taranan Top Coinler (Hacim Bazlı)</b></div>", unsafe_allow_html=True)
...
with col_sag:
    ...
    for log in st.session_state.ai_dusunce_gunlugu:
        log_kutusu.markdown(f"<div class='ai-log-box'>{log}</div>", unsafe_allow_html=True)
